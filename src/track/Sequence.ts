...
export class Sequence extends Array<TrackChord> {
    ...
    static from<T extends TrackChord>(arrayLike: Iterable<T> | ArrayLike<T>): T[];
    ...
    constructor(...items: TrackChord[]);
    constructor(p1?: number | TrackChord, ...arrayIn: TrackChord[]) {
    ...
    push(...items: TrackChord[]) {
        if (!isTrackChordArray(items)) throw new TypeError("Items to push are not TrackChords");
        return super.push(...items);
    }
    concat(...items: (TrackChord | ConcatArray<TrackChord>)[]) {
        if (!isTrackChordArray(items)) throw new TypeError("Items to concat are not TrackChords");
        return super.concat(...items);
    }
    unshift(...items: TrackChord[]) {
        if (!isTrackChordArray(items)) throw new TypeError("Items to unshift are not TrackChords");
        return super.unshift(...items);
    }
    fill(value: TrackChord, start?: number, end?: number) {
